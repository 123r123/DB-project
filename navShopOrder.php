...
if(!<?=$_SESSION['curUser']['identity']?>) {
    alert('You are not a shop owner yet, please register a shop!');
...
if ($status==="All") {
  $action="where SID=$SID";
} else {
  $action="where orderStatus='$status' and SID=$SID";
}
        ...
        <script>
          $(document).ready(function() {
            $("#cancel<?=$value['OID']?>").click( function() {           
              $.post("php/order/scancelOrder.php",{OID : <?=$value['OID']?>} ,function(data,status){
                $("#www").html(data);
                });        
             });
